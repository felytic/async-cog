...
@mark.asyncio
async def test_read_tile_image_jpeg(mocked_reader) -> None:
    async with mocked_reader("cog.tif") as reader:
        image = await reader.get_tile_image(0, 0, 0)
...
@mark.asyncio
async def test_read_tile_image_packbits(mocked_reader) -> None:
    async with mocked_reader("packbits.tif") as reader:
        image = await reader.get_tile_image(4, 0, 0)
